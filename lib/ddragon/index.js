...
module.exports.getDetailedChampionData = function(region, championId, cb) {
  async.waterfall([
    module.exports.currentPatch.bind(null, region),
    function getChampionData(currentPatch, cb) {
      module.exports.getChampionData(region, championId, rarity.carry([currentPatch], cb));
    },
    function downloadSpecificData(currentPatch, ddragonData, cb) {
      if(ddragonData.id === "Fiddlesticks") {
        ddragonData.id = "FiddleSticks";
      }
      downloadOrGetFromCache('/cdn/' + currentPatch + '/data/en_US/champion/' + ddragonData.id + '.json', cb);
    },
    function extractInterestingData(detailedData, cb) {
      cb(null, detailedData.data[Object.keys(detailedData.data)[0]]);
    }
  ], cb);
};
...
module.exports.getChampionData = function(region, championId, cb) {
  if(championId === "FiddleSticks") {
    // Patch 7.3.1 updated fiddle's name from "FiddleSticks" (capital S) to "Fiddlesticks" (lowercase s).
