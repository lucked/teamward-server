...
var downloadUser = function downloadUser(job, done) {

  debug("Downloading game list for user #" + job.accountId + " (" + job.region + ")");
  async.waterfall([
    function downloadList(cb) {
      gameInfo.getMatchList(job.accountId, job.region, cb);
    },
