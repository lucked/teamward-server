...
var workerFunction = newrelic.createBackgroundTransaction('download-game:download', 'workers', function workerFunction(job, cb) {
  ...
  var ApiMatch = mongoose.model('ApiMatch');

  async.waterfall([
    function downloadGameFromAPI(cb) {
      gameInfo.getExistingGameNoCache(job.id, job.region, cb);
    },
    function saveGameToMongo(apiMatchData, cb) {
      var match = Match.fromAPI(apiMatchData);
      var apiMatch = new ApiMatch();
      apiMatch._id = match._id;
      apiMatch.raw = apiMatchData;

      async.parallel([match.save.bind(match), apiMatch.save.bind(apiMatch)], cb);
    }], cb);
});
