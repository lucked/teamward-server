...
var workerFunction = newrelic.createBackgroundTransaction('download-game:download', 'workers', function workerFunction(job, done) {
  ...
  async.waterfall([
    function checkAlreadyAvailable(cb) {
      pool.query(queryToPg("SELECT id FROM matches WHERE id = ? AND region= ?"), [job.id, job.region], cb);
    },
    ...
    function saveGame(apiMatchData, txn, cb) {
      ...
      transaction.query(queryToPg("INSERT INTO matches(" + columns.join(',') + ") VALUES(" + placeholders + ")"), values, rarity.carry([apiMatchData], cb));
    },
    function savePlayers(apiMatchData, res, cb) {
      ...
      players.forEach(function(player) {
        ...
        values = values.map((v) => v === true ? 1 : (v === false ? 0 : v));
        var placeholders = columns.map(function() {
          return "?";
        }).join(",");
      ...
      console.log(values);
      transaction.query(queryToPg("INSERT INTO matches_participants(" + columns.join(',') + ") VALUES " + inserts.join(",")), values, cb);
    },
