...
var formatChampion = function(champion, allMatchupData, currentPatch, cb) {
  async.waterfall([
    ...
    function(ddragonChampion, cb) {
      var r = {
        ...
        passive: {
          ...
          image: 'http://ddragon.leagueoflegends.com/cdn/' + currentPatch + '/img/passive/' + ddragonChampion.passive.image.full
        },
  ...
  async.auto({
    ...
    formatData: ['warmCache', 'downloadRoles', 'downloadMatchup', function(res, cb) {
      async.map(res.downloadRoles, function(champion, cb) {
        formatChampion(champion, res.downloadMatchup, res.currentPatch, cb);
      }, cb);
    }],
