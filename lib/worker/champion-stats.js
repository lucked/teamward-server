...
module.exports = newrelic.createBackgroundTransaction('champion-stats:compute', 'workers', function()  {
  var pool = anyDB.createPool(config.sqlUrl, {min: 2, max: 5});
  ...
  async.auto({
    ...
    lastPatch: ['queryLastPatch', function lastPatch(res, cb) {
      log("Last patch with enough games is " + res.queryLastPatch.rows[0].patch);
      cb(null, res.queryLastPatch.rows[0]);
    }],
    queryChampionByLane: ['lastPatch', function queryChampionByLane(res, cb) {
      pool.query(laneSql, [res.lastPatch.patch, res.lastPatch.patch], cb);
    }],
    championByLane: ['queryChampionByLane', function championByLane(res, cb) {
      log("Found " + res.queryChampionByLane.rowCount + " champions/lanes combinaisons");
      cb(null, res.queryChampionByLane.rows);
    }],
    queryMatchups: ['championByLane', 'lastPatch', function queryMatchups(res, cb) {
      pool.query(matchupSql, [res.lastPatch.patch], cb);
    }],
    matchups: ['queryMatchups', function matchups(res, cb) {
      log("Found " + res.queryMatchups.rowCount + " matchups");
      ...
      async.reduce(res.championByLane, {}, function(champions, champion, cb) {
        ddragon.getChampionData('euw', champion.champion_id, function(err, ddragonChampion) {
          ...
          champions[champion.champion_id].roleData[roleName(champion.role)] = {
            ...
            gamesCount: champion.nb_games_in_role,
            percentPlayInRole: champion.percent_play_in_role
          };
    ...
    formatMatchups: ['formatLanes', 'matchups', function(res, cb) {
      ...
      res.matchups.forEach(function(matchup) {
        ...
        champions[matchup.champion1_id][key].push({
          games: matchup.nb_games,
          id: matchup.champion2_id,
          name: matchup.champion2_name,
          winRate: matchup.winrate
        });
      });

      cb(null, Object.keys(champions).map(id => champions[id]));
    }],
    storeData: ['formatMatchups', function(res, cb) {
      log("Updating database...");

      async.eachLimit(res.formatMatchups, 5, function(champion, cb) {
