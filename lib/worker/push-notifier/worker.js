...
module.exports = newrelic.createBackgroundTransaction('push-notifier:queue-worker', 'workers', function(job, ctx, cb) {
  ...
  async.waterfall([
    ...
    function logNotification(messageId, cb) {
      ...
      if(err && err.timeout) {
        // TIMEOUTs are bad, but not especially important
        err = null;
      }
