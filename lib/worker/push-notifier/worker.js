...
module.exports = newrelic.createBackgroundTransaction('push-notifier:checkInGame', 'workers', function(job, ctx, cb) {
  if(!cb) {
    cb = ctx;
    ctx = null;
  }
  ...
  async.waterfall([
    ...
    function sendNotification(res, cb) {
      ...
      module.exports.gcm.send(message, rarity.carry([res], cb));
    },
    function logNotification(res, messageId, cb) {
      debug(token.summonerName + " notified, message id " + messageId);
      notified = true;
      cb(null, res);
    },
    function preloadGameData(res, cb) {
      // Also prefetch game data, to cache them in case the user clicks on the notification
      gameData.buildExternalGameData(res, token.region, cb);
    }
  ], function(err) {
