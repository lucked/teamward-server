...
module.exports = function startQueue(options) {
  ...
  var refillQueueWorker = function(task, cb) {
    var lastLoopStartedAt = task.startedAt || new Date();
    ...
    async.waterfall([
      ...
      function queueNewRefillTask(refillTaskInQueue, cb) {
        if(refillTaskInQueue === 0 && options.loop) {
          debug("Adding new refillQueue task");
          queue.create('refillQueue', {startedAt: lastLoopStartedAt}).delay(500).ttl(5000).removeOnComplete(true).save(cb);
        }
        else {
          cb();
        }
      ...
      function addTokensToQueue(tokens, cb) {
        ...
        async.eachLimit(tokens, 10, function(token, cb) {
          queue.create('checkInGame', {token: token}).ttl(5000).removeOnComplete(true).save(rarity.carry([tokens.length], cb));
        }, cb);
      },
