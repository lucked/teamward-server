...
module.exports = function startQueue(options) {
  ...
  var addRefillQueueTaskIfNeeded = function(delay, cb) {
    queue.inactiveCount('refillQueue', function(err, total) {
      ...
      if(total === 0) {
        queue.create('refillQueue', {startedAt: new Date()}).delay(delay).removeOnComplete(true).save(cb);
      }
    });
  };
    ...
    queue.inactiveCount('checkInGame', function(err, total) {
      ...
      mongoose.model('Token').find({}).lean().exec(function(err, tokens) {
        async.eachLimit(tokens, 10, function(token, cb) {
          queue.create('checkInGame', {token: token}).removeOnComplete(true).save(cb);
        }, function(err) {
          if(err) {
            return cb(err);
          }
  ...
  var inGameProcessor = queue.process('checkInGame', QUEUE_CONCURRENCY, queueWorker);
  var refillQueueProcessor = queue.process('refillQueue', refillQueueWorker);
  ...
  if(!options.testing) {
    process.once('SIGTERM', function stopQueue() {
      ...
      async.parallel([
        function gameWorkerShutdown(cb) {
          inGameProcessor.shutdown(5000, cb);
        },
        function refillShutdown(cb) {
          refillQueueProcessor.shutdown(5000, cb);
        }
      ], function(err) {
        if(err) {
          errorLogger(err);
