    ...
    async.waterfall([
      ...
      function addTokensToQueue(tokens, cb) {
        ...
        async.eachLimit(tokens, 50, function(token, cb) {
          queue.create('checkInGame', {token: token, title: token.summonerName + " (" + token.region + ")"}).ttl(5000).removeOnComplete(true).save(cb);
      ...
      async.waterfall([
        ...
        function queueNewRefillTask(refillTaskInQueue, cb) {
          if(refillTaskInQueue === 0) {
            queue.create('refillQueue', {startedAt: tokenCount ? new Date() : lastLoopStartedAt}).delay(options.delay).ttl(15000).removeOnComplete(true).save(cb);
          }
          else {
            cb();
          }
