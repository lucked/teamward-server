  ...
  var defaultCb = function(err, tokenCounter, tokenNotifiedCounter) {
    ...
    if(options.loop) {
      var restartFunction = function() {
        ...
        debug("Looping over (" + tokenCounter + " tokens in " + timeToLoop + "s), notified " + tokenNotifiedCounter + " token" + (tokenNotifiedCounter > 1 ? "s" : ""));

        var threshold = 60;
        if(timeToLoop > threshold) {
          debug("More than " + threshold + "s to loop over " + tokenCounter + " tokens -- " + timeToLoop + "s :(");
        }
