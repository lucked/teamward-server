'use strict';
var QUEUE_CONCURRENCY = 15;
var QUEUE_MAX_SIZE = QUEUE_CONCURRENCY * 5;
    ...
    function getCurrentGame(cb) {
      ...
      gameInfo.getCurrentGame(token.summonerId, token.region, function(err, res) {
        if(err && err.statusCode === 404) {
          if(token.lastKnownGameId !== null) {
            // Summoner was in game, is not anymore
            var message = {
              registration_id: token.token,
              'data.removeGameId': res.gameId,
            };

            debug(token.summonerName + "(" + token.region + ") finished his game #" + res.gameId);

            // Send out of game message to device, to remove notification.
            gcm.send(message, noop);

            token.lastKnownGameId = null;
...
module.exports = function startWorker(opbeat) {
  ...
  var unpauseStream = function(err) {
    ...
    if(stream.paused && queue.length() <= QUEUE_CONCURRENCY) {
  ...
  stream.on('close', function() {
    ...
    var restartFunction = function() {
      ...
      debug("Looping over (" + tokenCounter + " tokens in " + Math.round(timeToLoop / 1000) + "s)!");
      lastLoopedAt = now;
      tokenCounter = 0;
      startWorker();
    };
