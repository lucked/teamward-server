...
function generateQueue(region) {
  var secondaryLog = require("debug")("teamward:riot-request:" + region);

  var requestQueue;

  // When using production key, make call in a faster way
  var defaultConcurrency = process.env.RIOT_API_KEY_IS_PRODUCTION ? 300 : 10;
      ...
      supertest("https://" + task.region + ".api.pvp.net")
        ...
        .end(function(err, res) {
          ...
          if(err && res && res.statusCode) {
            ...
            if(res.statusCode === 500 && !task.restartedAfter500) {
              task.restartedAfter500 = true;
              secondaryLog("Got a 500 on " + task.endpoint + ", will try again.");
              setTimeout(function() {
                queueWorker(task, cb);
              }, 25);
              return;
