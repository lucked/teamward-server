...
function queueWorker(task, cb) {
  log("Loading " + task.endpoint);
  supertest("https://" + config.server + ".api.pvp.net")
 ...
 */
module.exports = function riotRequest(endpoint, cacheable, done) {
  if(!done) {
    done = cacheable;
    cacheable = false;
  }
  ...
  async.waterfall([
    function checkCache(cb) {
      ...
      mongoose.model("HttpCache").findOne({endpoint: endpoint}, rarity.slice(2, cb));
    },
