...
module.exports.getSummonerData = function getSummonerData(summonerName, region, cb) {
  var endpoint = '/api/lol/' + region.toLowerCase() + '/v1.4/summoner/by-name/' + encodeURIComponent(summonerName.replace(/ /g, ''));

  riotRequest(region, endpoint, duration.SIX_HOURS, function(err, data) {
    if(err) {
...
module.exports.getCurrentRanks = function getCurrentRanks(summonerIds, region, cb) {
