...
module.exports.guessRoles = function(team, cb) {
  ...
  async.waterfall([
    ...
    function(championsRoles, cb) {
      ...
      var smiteFinder = function(player) {
        return (player.spell_d && player.spell_d.name === "Smite") || (player.spell_f && player.spell_f.name === "Smite");
      };

      var smiteCount = team.filter(smiteFinder).length;

      if(smiteCount === 1) {
        // We only have one Smite (which is a sane assumption),
        // We can assume this champion will play JUNGLE only
        var junglerPlayer = team.find(smiteFinder);

        championsRoles.forEach(function(championRole) {
          if(championRole._id.toString() === junglerPlayer.champion.id.toString()) {
            // Overwrite possible roles with the role we're sure he'll get
            championRole.roles = ['JUNGLE'];
          }
          else if(championRole.roles.indexOf("JUNGLE") !== -1) {
            // Remove JUNGLE from other champions
            championRole.roles.splice(championRole.roles.indexOf("JUNGLE"), 1);
          }
        });
      }
  ...
  while(hasFoundSomethingOnLastIteration) {
    ...
    for(var i = 0; i < team.length; i += 1) {
      ...
      if(champion._roles.length === 1) {
        ...
        // Affecting " + champion.name + " to " + role
