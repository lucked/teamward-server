...
module.exports.getPremade = function(premadeData) {
  ...
  Object.keys(premadeData).forEach(function(summonerId) {
    ...
    var potentialPremade = recursePremade(premadeData, new Set([summonerId.toString()]), summonerPremadeData.knownPlayers, new Set());
...
module.exports.loadKnownPremadesFromDB = function loadKnownPremadesFromDB(summonerIds, region, cb) {
  var prefix = region.toLowerCase() + ":";
  var lookingFor = summonerIds.map(function(id) {
    return prefix + id;
  });

  mongoose.model("Premade").find({_id: {$in: lookingFor}}, function(err, premades) {
    if(err) {
      return cb(err);
    }
...
module.exports.savePremadesToDB = function savePremadesToDB(premadesObject, region, cb) {
  var prefix = region.toLowerCase() + ":";
