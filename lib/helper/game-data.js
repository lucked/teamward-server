...
module.exports = function getGameData(summonerId, region, cb) {
  var secondaryLog = require("debug")("teamward:game-data:" + summonerId);

  log("Retrieving game data for summoner #" + summonerId + " (" + region + ")");
  ...
  async.auto({
    ...
    rawGameData: function loadCurrentGame(cb) {
      gameInfo.getCurrentGame(summonerId, region, function(err, res) {
    ...
    gameData: ['rawGameData', function ensureInGame(cb, res) {
      if(!res.rawGameData.gameId) {
        var error = new Error("Issues with the Riot API :( [CURR_GAME]");
    ...
    cb(err, {
      game_id: res.gameData.gameId,
      map_id: res.gameData.mapId,
      game_start_time: res.gameData.gameStartTime,
      game_mode: res.gameData.gameMode,
      game_type: res.gameData.gameType,
