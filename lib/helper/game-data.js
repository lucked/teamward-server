...
var matchInfo = require('../riot-api/match-info');
...
// Wrapperto keep going when some non essential part of the API are down
var errForgiver = function(cb) {
  return function(err, data) {
    if(err) {
      log("Got a non-blocking error", err);
      console.log("Got a non-blocking error", err);
  ...
  async.auto({
    ...
    rawSummonersChampionData: function rawSummonersChampionData(cb) {
      async.map(gameInfo.participants, function(participant, cb) {
        ...
        summonerInfo.getChampions(participant.summonerId, region, errForgiver(cb));
      }, cb);
    },
    ...
    rawMatchHistoryData: function rawMatchHistoryData(cb) {
      async.map(gameInfo.participants, function(participant, cb) {
        // This is not critical and can be skipped if this part of the API is down
        summonerInfo.getRecentMatches(participant.summonerId, region, errForgiver(cb));
      }, cb);
    },
    ...
    premadeData: ['rawMatchHistoryData', function premadeData(res, cb) {
      // First reduce: build an object,
      // keys are summonerId,
    ...
    lastSeasonRankedSummonerData: ['rankedSummonerData', 'matchHistoryData', function rawLastSeasonRankedSummonerData(res, cb) {
      ...
      async.map(gameInfo.participants, function(participant, cb) {
        ...
        if(matchHistory && matchHistory.games && matchHistory.games.length > 0) {
          ...
          matchInfo.getMatchData(sampleMatchId, region, function(err, matchData) {
            if(err) {
    ...
    finalResult: ['summonersChampionData', 'rankedSummonerData', 'preloadChampionData', 'summonersData', 'matchHistoryData', 'lastSeasonRankedSummonerData', function buildFinalResult(res, cb) {
