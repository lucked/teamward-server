...
module.exports = function getGameData(summonerName, region, cb) {
  var secondaryLog = require("debug")("teamward:game-data:" + summonerName.toLowerCase());
  ...
  async.auto({
    ...
    rawGameData: ['mainSummonerData', function loadCurrentGame(cb, res) {
      if(!res.mainSummonerData) {
        var error2 = new Error("Issues with the Riot API :( [MISS_SUMM_DATA]");
        error2.riotInternal = true;
        return cb(error2);
      }
      ...
      gameInfo.getCurrentGame(res.mainSummonerData.id, region, function(err, res) {
    ...
    gameData: ['rawGameData', function ensureInGame(cb, res) {
      if(!res.rawGameData.gameId) {
        var error2 = new Error("Issues with the Riot API :( [CURR_GAME]");
        error2.riotInternal = true;
        return cb(error2);
      }
      else {
